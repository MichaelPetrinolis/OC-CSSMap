...
prettify = function (es) {
    return es.map(function (file, cb) {       
        if (file.path.indexOf(".min.") > 0) {
            util.log('Is minified');
            var newPath = file.path.replace(".min.", ".");
            util.log(this);
            return cb(null, gulp.src(this).pipe(prettier()));
        }
        return cb(null, gulp.pipe(file));
    });
};
...
function buildJsPipeline(assetGroup, doConcat, doRebuild) {
    ...
    var minify = assetGroup.hasOwnProperty("minify") ? assetGroup.minify : true;
    var generateSourceMaps = assetGroup.hasOwnProperty("generateSourceMaps") ? assetGroup.generateSourceMaps : true;
    // Source maps are useless if neither concatenating nor transforming.
    ...
    return gulp.src(assetGroup.inputPaths)
        ...
        .pipe(gulpif(minify, eol()))
        .pipe(gulpif(minify, gulp.dest(assetGroup.outputDir)));
    // Uncomment to copy assets to wwwroot
    //.pipe(gulp.dest(assetGroup.webroot));
}
